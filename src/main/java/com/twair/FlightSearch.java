...
public class FlightSearch {
    ...
    public FlightSearch byFlightClass(String flightClass, Integer noOfSeats) throws InvalidFlightclassException {
        if (flightClass == null){
            throw new InvalidFlightclassException();
        }

        List<Flight> matchingFlights = new ArrayList<Flight>();
        for (Flight flight : flightList) {
            if (flight.getSeatsByClass(flightClass) > noOfSeats) {
                matchingFlights.add(flight);
            }
        }
        return new FlightSearch(matchingFlights);
