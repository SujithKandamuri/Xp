...
public class TwAirApplication {
	...
	public String search(@ModelAttribute(value="searchForm") SearchForm searchForm, Model model) throws Exception , InvalidFlightclassException{
		model.addAttribute("locations", DataSource.instance().fetchLocations());
		try {
			...
			matchingFlights = matchingFlights.byFlightClass(searchForm.getFlightClass(),searchForm.getNumberSeats());
